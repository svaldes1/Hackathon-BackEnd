...
df = pd.DataFrame()
df = generateSkewSample(0, 50, 10, 5000, 'Spend_Power', df)
df = generateSkewSample(0, 0.5, 0.2, 5000, 'Tech_Sav', df)
df = generateSkewSample(3, 27, 7, 5000, 'Age', df)
df = generateSkewSample(1, 2, 0.5, 5000, 'Time_Cust', df)
df = generateSkewSample(-1, 8, 0.5, 5000, 'Satisfaction', df)
df = generateSkewSample(-1, 3.5, 0.5, 5000, 'House_Size', df)
df = generateSkewSample(10, 1, 0.5, 5000, 'Latest_Plan', df)
test = product_funct(0.01, 0.25, 0.2, df)
test2 = accessory_funct(0.3, 0.4, 0.25, df)
...
# export df to csv
df.to_csv('budgetshopdata.csv', index=False)
