...
df = pd.DataFrame()
df = generateSkewSample(0, 100, 15, 5000, 'Spend_Power', df)
df = generateSkewSample(-6, 1, 0.1, 5000, 'Tech_Sav', df)
df = generateSkewSample(3, 25, 10, 5000, 'Age', df)
df = generateSkewSample(4, 2, 6, 5000, 'Time_Cust', df)
df = generateSkewSample(-3, 8, 2, 5000, 'Satisfaction', df)
df = generateSkewSample(5, 1.5, 0.75, 5000, 'House_Size', df)
df = generateSkewSample(-3, 2.0, 0.7, 5000, 'Latest_Plan', df)
test = product_funct(0.3, 0.3, 0.3, df)
...
# export df to csv
df.to_csv('techgeekdata.csv', index=False)
print("Done!")
