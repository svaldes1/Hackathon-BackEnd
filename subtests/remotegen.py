...
df = pd.DataFrame()
df = generateSkewSample(0, 160, 20, 5000, 'Spend_Power', df)
df = generateSkewSample(0.5, 0.6, 0.15, 5000, 'Tech_Sav', df)
df = generateSkewSample(3.5, 25, 4.5, 5000, 'Age', df)
df = generateSkewSample(1.5, 4, 0.5, 5000, 'Time_Cust', df)
df = generateSkewSample(0, 5, 1.25, 5000, 'Satisfaction', df)
df = generateSkewSample(-2, 3, 0.5, 5000, 'House_Size', df)
df = generateSkewSample(-10, 3.75, 0.6, 5000, 'Latest_Plan', df)
test = product_funct(0.7, 0.2, 0.1, df)
test2 = accessory_funct(0.05, 0.1, 0.2, df)
df = generateSkewSample(-1.5, 7, 2.2, 5000, 'Phone_Plan', df)
df = generateSkewSample(-1.5, 7, 0.95, 5000, 'Home_Plan', df)
...
# export df to csv
df.to_csv('remoteworkdata.csv', index=False)
