...
df = pd.DataFrame()
df = generateSkewSample(0, 90, 25, 5000, 'Spend_Power', df)
df = generateSkewSample(4, 0.2, 0.2, 5000, 'Tech_Sav', df)
df = generateSkewSample(4, 70, 8, 5000, 'Age', df)
df = generateSkewSample(2.5, 3, 15, 5000, 'Time_Cust', df)
df = generateSkewSample(0.2, 3, 2, 5000, 'Satisfaction', df)
df = generateSkewSample(-0.1, 1.5, 1, 5000, 'House_Size', df)
df = generateSkewSample(-0.5, 2.0, 0.5, 5000, 'Latest_Plan', df)
test = product_funct(0.2, 0.4, 0.1, df)
test2 = accessory_funct(0.1, 0.1, 0.2, df)
df = generateSkewSample(0, 5, 2.2, 5000, 'Phone_Plan', df)
df = generateSkewSample(0, 5, 0.95, 5000, 'Home_Plan', df)
...
# export df to csv
df.to_csv('SeniorCitizendata.csv', index=False)
print("Done!")