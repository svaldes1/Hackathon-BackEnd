...
df = pd.DataFrame()
df = generateSkewSample(0, 170, 30, 5000, 'Spend_Power', df)
df = generateSkewSample(0, 0.6, 0.2, 5000, 'Tech_Sav', df)
df = generateSkewSample(0, 35, 6, 5000, 'Age', df)
df = generateSkewSample(3, 2, 2, 5000, 'Time_Cust', df)
df = generateSkewSample(-4, 9, 3, 5000, 'Satisfaction', df)
df = generateSkewSample(5, 1.5, 1, 5000, 'House_Size', df)
df = generateSkewSample(-0.5, 2.6, 0.7, 5000, 'Latest_Plan', df)
test = product_funct(0.7, 0.05, 0.2, df)
test2 = accessory_funct(0.2, 0.2, 0.3, df)
df = generateSkewSample(-0.5, 7, 2, 5000, 'Phone_Plan', df)
df = generateSkewSample(-0.5, 5, 0.95, 5000, 'Home_Plan', df)
...
# export df to csv
df.to_csv('businessCitydata.csv', index=False)
print("Done!")